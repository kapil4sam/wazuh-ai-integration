...
def create_vectorstore(logs, embedding_model):
    ...
    docs = []
    for log in logs:
        content = log.get('full_log') or json.dumps(log)
...
def initialize_assistant_context():
    return (
        "You are a security analyst performing threat hunting on Wazuh logs stored in "
        "the vector store. The objective is to identify potential security threats or any "
        "other needs from the user. All queries should be interpreted as asking about "
        "security events, patterns or other request from the user using the vectorized logs."
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            result = qa_chain({"question": data, "chat_history": chat_history})
            answer = result.get("answer", "").replace("\\n", "\n").strip()
            if not answer:
                answer = "⚠️ Sorry, I couldn't generate a response."

            chat_history.append((data, answer))
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .topbar {
    ...
    padding: 14px 24px; background: #2a2a2a;
    border-bottom: 1px solid #333; flex-shrink: 0;
  }
  .topbar-icon {
    width: 32px; height: 32px; background: #10a37f;
  ...
  .status-dot {
    ...
    background: #555; margin-left: 6px; flex-shrink: 0; transition: background 0.3s;
  }
  ...
  .row {
    ...
    align-items: flex-start; max-width: 860px; width: 100%; margin: 0 auto;
  }
  ...
  .bubble.thinking {
    color: #8e8ea0; font-style: italic; background: transparent;
    padding-left: 0; animation: pulse 1.4s ease-in-out infinite;
  }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
  .empty-state {
    flex: 1; display: flex; flex-direction: column;
    align-items: center; justify-content: center;
    gap: 10px; color: #555; user-select: none;
  }
  .empty-state .icon { font-size: 40px; }
  .empty-state p { font-size: 14px; }
  .input-wrap { flex-shrink: 0; padding: 16px 24px 20px; background: #212121; }
  .input-box {
    max-width: 860px; margin: 0 auto; display: flex; align-items: flex-end;
    gap: 8px; background: #2a2a2a; border: 1px solid #444;
    border-radius: 16px; padding: 10px 12px 10px 16px; transition: border-color 0.2s;
  }
  .input-box:focus-within { border-color: #10a37f; }
  textarea {
    flex: 1; background: transparent; border: none; outline: none;
    color: #ececec; font-family: inherit; font-size: 14px;
    line-height: 1.5; resize: none; max-height: 160px; overflow-y: auto; padding: 2px 0;
  }
  textarea::placeholder { color: #666; }
  .send-btn {
    width: 34px; height: 34px; flex-shrink: 0; background: #10a37f;
...
<div class="topbar">
  ...
  <span class="topbar-sub">Powered by Ollama (Llama 3)</span>
...
<script>
  ...
  socket.onopen  = () => { statusDot.classList.add('connected'); sendBtn.disabled = !userInput.value.trim(); };
  ...
  socket.onerror = () => { removeThinking(); appendMessage('bot', '⚠️ WebSocket error.'); awaitingReply = false; };

  function appendMessage(role, text) {
    if (emptyState) emptyState.remove();
    const row = document.createElement('div'); row.className = 'row ' + role;
    const avatar = document.createElement('div'); avatar.className = 'avatar'; avatar.textContent = role === 'bot' ? 'W' : 'U';
  ...
  function showThinking() {
    ...
    const avatar = document.createElement('div'); avatar.className = 'avatar'; avatar.textContent = 'W';
    const bubble = document.createElement('div'); bubble.className = 'bubble thinking'; bubble.textContent = 'Analyzing logs…';
    row.appendChild(avatar); row.appendChild(bubble);
    messagesDiv.appendChild(row); messagesDiv.scrollTop = messagesDiv.scrollHeight;
    thinkingRow = row;
  }
  function removeThinking() { if (thinkingRow) { thinkingRow.remove(); thinkingRow = null; } }
  function sendMessage() {
    ...
    sendBtn.disabled = true; awaitingReply = true; showThinking(); userInput.focus();
  }
